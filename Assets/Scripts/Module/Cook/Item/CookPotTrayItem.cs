        ...
        IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        public void Bind(CookMaterialData material, bool isFull = false, float trayPreviewScore = 0f)
        {
            ...
            if (_imgIcon != null)
            {
                ...
                CookMaterialIconVisual.Apply(_imgIcon, new Vector2(92f, 92f), _imgIcon.rectTransform);
            }
        ...
        // 取出暂存槽上正在显示的材料图标，供投入飞行动画复用（保持当前屏幕位置）
        public bool TryTakeIconForSubmitFly(out RectTransform iconRt)
        {
            iconRt = null;
            ensureReferences();

            if (!_hasMaterial || _imgIcon == null || _imgIcon.sprite == null || !_imgIcon.enabled)
                return false;

            stopFlash();
            iconRt = _imgIcon.rectTransform;
